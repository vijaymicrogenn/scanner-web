...
export default function ViewNationality() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get("/api/nationality/view");
  ...
  const handleSave = async () => {
    if (!editValue.trim()) {
      showToast("Nationality name cannot be empty", "error");
      return;
    }

    setIsSubmitting(true);
    try {
      await axios.put(`/api/nationality/update/${editId}`, {
        nationality_name: editValue,
      });
      showToast("Nationality updated successfully....");
      handleCancel();
      fetchData();
    } catch (error) {
      if (error.response?.status === 409) {
        showToast("Nationality name already exists", "error");
      } else {
  ...
  const confirmSoftDelete = async () => {
    setIsSubmitting(true);
    try {
      await axios.put(`/api/nationality/soft-delete/${deleteId}`);
      showToast("Nationality deactivated successfully");
      fetchData();
    } catch {
        ...
        <table className="table table-dark table-hover mb-0">
          <thead style={{
            ...
            <tr>
              ...
              <th style={{ padding: "12px" }}>Nationality Name</th>
          ...
          <tbody>
            {data.length === 0 ? (
              ...
              data.map((item, i) => (
                <tr  key={item.nationality_id} style={{ borderColor: "rgba(255,255,255,0.1)" }}>
                  <td className="text-center p-1" style={{ padding: "12px" }}>{i + 1}</td>
                  <td className="p-1" style={{ padding: "12px" }}>{item.nationality_name}</td>
                  <td className=" p-1 text-center" style={{ padding: "12px" }}>
                    <FontAwesomeIcon
                      icon={faEdit}
                      className="text-warning mx-2 p-0"
                      style={{ cursor: "pointer", fontSize: "16px" }}
                      onClick={() => handleEdit(item.nationality_id, item.nationality_name)}
                    ...
                    <FontAwesomeIcon
                      ...
                      style={{ cursor: "pointer", fontSize: "16px" }}
                      onClick={() => handleSoftDelete(item.nationality_id)}
                      title="Deactivate Nationality"
                    />
                  </td>
                </tr>
              ))
          ...
          <div className="modal-dialog modal-sm modal-dialog-centered">
            <div className="modal-content" style={{
              ...
              <div className="modal-header py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
                <h6 className="modal-title text-white fw-semibold">Edit Nationality</h6>
                <FontAwesomeIcon
                  icon={faTimes}
        ...
        <div className="modal fade show d-block" tabIndex="-1" style={{ backgroundColor: "rgba(0,0,0,0.7)", zIndex: 1050 }}>
          <div className="modal-dialog modal-sm modal-dialog-centered">
            <div className="modal-content" style={{
              ...
              <div className="modal-body py-3">
                <p className="small m-0 text-light">Deactivate this nationality? It will be hidden from active lists.</p>
              </div>
              <div className="modal-footer py-3" style={{ borderTop: "1px solid rgba(255,255,255,0.1)" }}>
                <button
